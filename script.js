...
//}
$(function () {
    //$('.form-group').children('input').each(function () {
    //    console.log(this)
    //})
...
function checkLengthMax8() {
    var length = $($(this)).val().length
    console.log(length)
    if (length < 0) {
        changeBorderColor('red',$(this))
        displayMessage($(this), 'Not enough characters')
        return false
    } else if (length > 8) {
        changeBorderColor('red',$(this))
        displayMessage($(this), 'Too many characters')
        return false
    }
    else {
        changeBorderColor('green', $(this))
        $('.msg').remove()
    }
}function checkLengthMin10Max25() {
    var length = $($(this)).val().length
    console.log(length)
    if (length < 10) {
        changeBorderColor('red',$(this))
